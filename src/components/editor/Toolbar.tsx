...
import { CodeLanguage, CodeTemplateCategory } from "@/shared/schema";
...
export function Toolbar({
  ...
  const templateOptions = templates
    .filter((temp) => temp.category === activeCategory).map(template => ({
      value: template.id,
            ...
            {onPanelChange && (
              <div className="flex border border-neutral-200 dark:border-neutral-700 rounded-md overflow-hidden">
                <button
                  ...
                  className={`px-3 py-1.5 text-sm flex items-center ${activePanel === 'editor'
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-sm flex items-center ${activePanel === 'results'
                    ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300'
                    : 'bg-white dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300'
                    }`}
                >
